...
data_lr.preprocess(standardization=True, degree_of_polynomial=DEGREE_OF_POLYNOMIAL)

# feature selection
data_lr.feature_selection(estimator=linear_model.LinearRegression(),
                          method=FEATURE_SELECTION,
                          num_fs_wanted=NUM_OF_FEATURES_WANTED)
print(data_lr.selected_features)
print('number of selected significant features', len(data_lr.selected_features))

# train & predict
multi_linear_model = MultiLinearReg(df=data_lr.df, features=data_lr.selected_features, y_name=data_lr.y_name)
...
data_nn.preprocess(standardization=True)      # I did not add polynomial terms for neural network model
# feature selection
...
data_nn.feature_selection(estimator=MLPRegressor(activation=ACTIVATION,
                          ...
                          num_fs_wanted=NUM_OF_FEATURES_WANTED)
print(data_nn.selected_features)
print('number of selected significant features', len(data_nn.selected_features))

# train & predict
multi_nn_model = MultiNNRegression(df=data_nn.df, features=data_nn.selected_features, y_name=data_nn.y_name)
